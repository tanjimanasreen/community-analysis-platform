...
def find_matching_communities(absolute_community_df, weighted_community_df):
    if absolute_community_df.empty or weighted_community_df.empty:
        return (
            pd.DataFrame(columns=['abs_community', 'per_community', 'jaccard_score', 'members']),
            pd.DataFrame(columns=['abs_community', 'absolute_members', 'per_community', 'weighted_members', 'jaccard_score', 'common_members', 'uncommon_members']),
        ...
        for index2, row2 in df2.iterrows():
            ...
            if jscore == 1:
                ...
                partial_matched.append((
                    row1['community_number'], row1['members'], 
                    row2['community_number'], row2['members'], 
                    jscore, common_members, uncommon_members
                ))
                if row1['community_number'] in unmatched_df1:
                    unmatched_df1.remove(row1['community_number'])
                if row2['community_number'] in unmatched_df2:
    ...
    partial_matched_df = pd.DataFrame(partial_matched, columns=['abs_community', 'absolute_members', 'per_community', 'weighted_members', 'jaccard_score', 'common_members', 'uncommon_members'])
