...
def get_specific_community_info(df_community, community_number, df_network, df_user):
    ...
    community_details = pd.DataFrame(columns=[
        'producer_username', 'producer_user_id',
    ...
    for index, row in community.iterrows():
        ...
        text_col = 'text_translated' if 'text_translated' in result_rows.columns else 'text'
        messages = list(result_rows[text_col]) if text_col in result_rows.columns else []
    ...
    if rows_to_append:
        community_details = pd.concat([community_details, pd.DataFrame(rows_to_append)], ignore_index=True)
...
def get_community_messages(prominent_communities, community_df, df_network, df_user):
    ...
    for i in range(len(prominent_communities)):
        specific_community_result = get_specific_community_info(community_df, i, df_network, df_user)

        # For dataframes with list columns, sum() concatenates the lists
        messages_combined = specific_community_result['messages'].sum() if not specific_community_result.empty else []
        message_ids_combined = specific_community_result['message_ids'].sum() if not specific_community_result.empty else []
        total_messages = specific_community_result['total_messages'].sum() if not specific_community_result.empty else 0
    ...
    if rows_to_append:
        community_messages = pd.concat([community_messages, pd.DataFrame(rows_to_append)], ignore_index=True)
...
def get_overall_community_messages_stat(date_col, promiment_communities, df_community, df_network):
    ...
    community_messages_date['day'] = community_messages_date[date_col].apply(_extract_day)
