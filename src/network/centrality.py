...
def show_hightest_out_degree(G, df_user):
    ...
    out_node = (
        sorted(out_degree_centrality.items(), key=lambda item: item[1], reverse=True)
    )[:5]
    ...
    average_centrality = sum(out_degree_centrality.values()) / len(
...
def show_hightest_in_degree(G, df_user):
    ...
    in_node = (
        sorted(in_degree_centrality.items(), key=lambda item: item[1], reverse=True)
    )[:5]
