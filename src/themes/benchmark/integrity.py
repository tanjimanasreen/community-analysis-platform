...
EXPECTED_PHASE1_DATASET_HASH = (
    "6b553bc95a601620d752b7763a5f7caea82b50ac2329279da4f22b17e9278951"
)
EXPECTED_PHASE1_SPLIT_HASH = (
    "5065183328ace9fddd39df8d588e8f065c1b99d34373f905ec1f97d857424200"
)
...
def validate_frozen_dataset(
    ...
    *,
    ...
    duplicates = sorted(
        item for item, count in Counter(example_ids).items() if count > 1
    )
    if duplicates:
        raise ThemeBenchmarkError(
            f"Frozen dataset contains duplicate example IDs: {duplicates[:5]}"
        )
    ...
    for row in examples:
        ...
        if split not in VALID_SPLITS:
            raise ThemeBenchmarkError(
                f"Example {example_id} has unsupported split {split!r}."
            )
        existing = split_by_example.get(example_id)
        if existing is not None and existing != split:
            raise ThemeBenchmarkError(
                f"Example {example_id} appears in multiple splits."
            )
        split_by_example[example_id] = split
        for field in ORIGINAL_KEYWORD_FIELDS:
            if field not in row:
                raise ThemeBenchmarkError(
                    f"Example {example_id} is missing original keyword field {field}."
                )
        if not row.get("general_keywords") or not row.get("general_keyword_text"):
            raise ThemeBenchmarkError(
                f"Example {example_id} is missing production-equivalent general keywords."
            )
    ...
    actual_counts = {
        ...
        for split in set(manifest_counts) | set(split_counts)
    }
...
def filter_examples_by_split(
    example_ids: list[str], dataset_rows: list[Mapping[str, Any]], split: str | None
) -> set[str] | None:
    if split is None:
        return None
    if split not in VALID_SPLITS:
        raise ThemeBenchmarkError(
            f"Unsupported benchmark split {split!r}; expected one of {VALID_SPLITS}."
        )
    split_ids = {
        str(row["example_id"]) for row in dataset_rows if row.get("split") == split
    }
    known_ids = set(example_ids)
    missing = sorted(split_ids.difference(known_ids))
    if missing:
        raise ThemeBenchmarkError(
            f"Split {split!r} references examples missing from requests: {missing[:5]}"
        )
    return split_ids


def _validate_source_artifacts(
    manifest: Mapping[str, Any], examples: list[Mapping[str, Any]]
) -> dict[str, Any]:
    ...
    for row in examples:
        ...
        if not root or not filename:
            raise ThemeBenchmarkError(
                f"Example {example_id} is missing source artifact metadata."
            )
        source_path = str(Path(str(root)) / str(filename))
        if source_path not in source_rows:
            raise ThemeBenchmarkError(
                f"Example {example_id} references unknown source artifact {source_path}."
            )
        index = int(row.get("source_row_index", -1))
        if index < 0 or index >= source_rows[source_path]:
            raise ThemeBenchmarkError(
                f"Example {example_id} references invalid source row {index}."
            )
...
def _distribution_report(
    ...
    *,
    ...
    by_split = {
        split: [row for row in examples if row.get("split") == split]
    ...
    for split, rows in by_split.items():
        dimensions["month"][split] = dict(
            Counter(str(row.get("month")) for row in rows)
        )
        dimensions["community_size_bin"][split] = dict(
            Counter(_member_bin(row) for row in rows)
        )
        dimensions["keyword_richness_bin"][split] = dict(
            Counter(_keyword_bin(row) for row in rows)
        )
        dimensions["absolute_community_coverage"][split] = dict(
            Counter(
                (
                    "present"
                    if row.get("absolute_community") not in (None, "")
                    else "missing"
                )
                for row in rows
            )
        ...
        dimensions["weighted_community_coverage"][split] = dict(
            Counter(
                (
                    "present"
                    if row.get("weighted_community") not in (None, "")
                    else "missing"
                )
                for row in rows
            )
        )
        dimensions["source_artifact_root"][split] = dict(
            Counter(str(row.get("source_artifact_root")) for row in rows)
        )
    ...
    if inventory_path.exists():
        inventory = read_json(inventory_path)
        for source in inventory.get("sources", []):
            rejected_by_source[str(source.get("input_dir"))] = int(
                source.get("rejected_row_count", 0) or 0
            )
...
def _reject_nonstandard_json(value: Any, label: str) -> None:
    try:
        ...
        raise ThemeBenchmarkError(
            f"Non-standard JSON value found in {label}: {exc}"
        ) from exc
