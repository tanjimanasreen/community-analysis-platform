...
from src.themes.benchmark.contracts import REVIEW_SCHEMA_VERSION, ThemeBenchmarkError, read_jsonl, stable_hash
...
def export_blinded_review(output_base_path: str | Path, run_id: str) -> Path:
    ...
    pd.DataFrame(columns=["review_id", *REVIEW_SCORE_COLUMNS, "notes"]).to_csv(template_path, index=False)
    return out_path
...
def validate_review_import(path: str | Path) -> None:
    ...
    if missing:
        raise ThemeBenchmarkError(f"Review import is missing required columns: {missing}")
    ...
    if leaked_columns:
        raise ThemeBenchmarkError(f"Review import must not include provider identity columns: {leaked_columns}")
    ...
    if leaked:
        raise ThemeBenchmarkError("Review import appears to contain provider identity text.")
    for column in REVIEW_SCORE_COLUMNS:
        values = pd.to_numeric(frame[column], errors="coerce")
        if values.isna().any():
            raise ThemeBenchmarkError(f"Review import column {column!r} must contain numeric scores.")
        invalid = values[(values < 1) | (values > 5)]
        if not invalid.empty:
            raise ThemeBenchmarkError(f"Review import column {column!r} must contain scores from 1 to 5.")
