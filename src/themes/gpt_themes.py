...
def map_theme(
    community_value, reference_df, reference_community_column, theme_gpt, theme_names
):
...
def extract_unique_keywords(month_df):
    ...
    df["all_keywords"] = (
        df["absolute_unigram_keywords"]
        + df["absolute_bigram_keywords"]
        + df["weighted_unigram_keywords"]
        + df["weighted_bigram_keywords"]
    )
    df["all_keywords"] = df["all_keywords"].apply(lambda x: list(dict.fromkeys(x)))
    df["all_keywords"] = df["all_keywords"].apply(
        lambda x: str(x)
        .replace("[", "")
        .replace("]", "")
        .replace(" ", "")
    ...
    df["absolute_keywords"] = (
        df["absolute_unigram_keywords"] + df["absolute_bigram_keywords"]
    )
    df["absolute_keywords"] = df["absolute_keywords"].apply(
    ...
    df["weighted_keywords"] = (
        df["weighted_unigram_keywords"] + df["weighted_bigram_keywords"]
    )
    df["weighted_keywords"] = df["weighted_keywords"].apply(
        lambda x: list(dict.fromkeys(x))
    )
    df["weighted_keywords"] = df["weighted_keywords"].apply(
        lambda x: str(x)
...
def generate_llm_themes(provider: LLMProvider, month_df: pd.DataFrame):
    ...
    absolute_grouped_df = (
        absolute_temp_df.groupby("absolute_keywords")
        .agg({"absolute_community": list})
        .reset_index()
    )

    weighted_temp_df = df[["weighted_community", "weighted_keywords"]]
    weighted_grouped_df = (
        weighted_temp_df.groupby("weighted_keywords")
        .agg({"weighted_community": list})
        .reset_index()
    )
    ...
    for ind, row in absolute_grouped_df.iterrows():
        ...
        absolute_grouped_df.at[ind, "absolute_theme_names"] = ".".join(
            list(gpt_result_abs.keys())
        )
    ...
    for ind, row in weighted_grouped_df.iterrows():
        ...
        weighted_grouped_df.at[ind, "weighted_theme_names"] = ".".join(
            list(gpt_result_wei.keys())
        )
