...
def get_matched_topic_df(
    lda_models, dfs, community_id_pairs, num_topics=15, top_n_keywords=50
):
    lda_titles = [
        "absolute_unigram",
        "weighted_unigram",
    ...
    for absolute_id, weighted_id in community_id_pairs:
        ...
        for i, (lda_model, df, community_id) in enumerate(
            zip(lda_models, dfs, community_ids_ordered)
        ):
            ...
            if topic_id == -1:
                ...
                topic_keywords_data = lda_model.show_topic(
                    topic_id, topn=top_n_keywords
                )
                ...
                keywords = [
                    word
                    for word, prob in topic_keywords_data
