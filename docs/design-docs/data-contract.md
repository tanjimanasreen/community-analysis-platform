...
| `MonthlyThemeCluster`     | `period: str`, `monthly_cluster_id: str`                    | HDBSCAN cluster over field-specifically parsed `general_theme_names`; commas are semantic label text, legacy dot-joined multi-theme strings require exact `general_theme_gpt` round-trip reconstruction, and noise is evidence-only. Monthly clustering contract `3.0`. |
|                          | `representative_theme: str`, `source_theme_labels: List[str]`   | Representative is a deterministic semantic medoid. Raw recorded embeddings remain unnormalized; Stage A clusters an in-memory L2-normalized copy with Euclidean distance, leaf selection, default `min_samples=3`, and `allow_single_cluster=false`. |
|                          | `clustering_input_normalized: bool`, `hdbscan_min_samples: int` | Provenance distinguishes normalized Stage-A geometry from the raw embedding artifact and records effective density. |
|                          | `hdbscan_cluster_selection_method: str`, `hdbscan_allow_single_cluster: bool` | Production contract `3.0` records `leaf` and `false`. |
