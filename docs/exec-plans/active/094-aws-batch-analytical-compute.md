...
- 2026-09-10: Fixed AWS Batch raw-input staging defect revealed by Telegram canonical run cdb47703-5515-4951-9705-42bb6c76f318 (where syncing entire S3 raw/ prefix exhausted the 30 GiB container ephemeral disk). Implemented resolve_raw_input_staging_targets() and download_config_raw_inputs() in src/cloud/batch_runner.py to download ONLY config-referenced input paths under raw/, mapping to ${DATA_ROOT} container destinations (/app/workspace/data/raw/<rel_path>). Confirmed fail-fast FileNotFoundError for missing S3 objects, selective staging for Twitter reply (4 files) and Twitter retweet_quote (4 files), exactly 10 files (17,832,762,551 bytes / ~16.61 GiB) and 0 Twitter objects for Telegram canonical run, and preservation of translation-cache staging. Verified with 32 unit tests in tests/unit/test_batch_runner.py, make lint, make infra-test (93 passed), make ci-validate (814 passed), and clean git diff --check.
- 2026-09-11: Finished canonical Telegram evolution run 4c049946-e6a0-4bf0-a9e6-e13f8d02a58f on AWS Batch (job baa1811d-0493-4e92-8bad-bc5399e8d302), successfully executing all 10 months (Jan–Oct 2019, 16.61 GiB raw data, 209,814 network interactions, 225 monthly theme rows, 90 monthly theme clusters, 67 canonical families, 26 transitions, 41 community paths, 267 path theme similarities). Overhauled repository root README.md into a production-grade portfolio and recruiter-facing landing page documenting current deployed AWS architecture (Batch Fargate compute, S3 Parquet manifests, Lambda/API Gateway, CloudFront, Vite/React), analytical methodology, real-data validation results, and local reproducible workflows.
- 2026-09-12: Completed the final recruiter-facing portfolio pass of root README.md. Structured narrative around ML/DS depth: 30-90s executive summary, ML highlights, prominent real-data validation (Telegram 10 months 2019, Twitter Retweet/Quote 4 months 2017), 2x2 real dashboard screenshots in docs/images/, three professional diagrams (ML/analytical pipeline, production ML system architecture, longitudinal community intelligence), mathematical methods, verified tech stack, concise quick start, and research-to-production evolution comparison.
