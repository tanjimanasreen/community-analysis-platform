...
def test_run_full_pipeline_can_skip_topic_phase(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        pipeline,
        'run_network_phase',
        lambda **kwargs: ('absolute_graph', 'weighted_graph', pd.DataFrame({'id': [1]}), pd.DataFrame({'user_id': [1]})),
    )
    monkeypatch.setattr(
        pipeline,
        'run_community_phase',
...
def test_run_full_pipeline_runs_topic_phase_by_default(monkeypatch, tmp_path):
    ...
    assert pipeline.run_full_pipeline(pd.DataFrame({'source': [1]}), output_dir=str(tmp_path))
    assert len(calls) == 1
...
def test_run_topic_phase_writes_theme_input_artifacts(monkeypatch, tmp_path):
    ...
    internal_csv = tmp_path / 'twitter' / '_intermediate' / 'theme_inputs' / 'reply' / '2017' / '03_2017.csv'
    manifest = internal_csv.parent / 'manifest.json'

    assert public_csv.exists()
    assert internal_csv.exists()
    assert manifest.exists()
    assert list(pd.read_csv(public_csv).columns) == list(pd.read_csv(internal_csv).columns)
